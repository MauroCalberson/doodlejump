...
// Handle collisions between player and platforms/bonuses
    void World::handleCollisions() {
        auto collision = CollisionSystem::detectCollision(player, platformManager->getPlatforms(),
                                                          bonusManager->getBonuses());
...
// Calculate coordinates for a new platform
    std::pair<float, float> World::calcPlatformCoords() {
        ...
        float y =
                platformManager->getMaxPlatformHeight() - baseSpacing - (randVariation * difficulty) - 50 * difficulty;
...
// Notify all entity observers of their updated positions
    void World::notifyEntityObservers() {
        ...
        entities.insert(entities.end(), bgTileManager->getBackgroundTiles().begin(),
                        bgTileManager->getBackgroundTiles().end());
